...
**Admin account bootstrapping.** Public registration always assigns `ROLE_USER` — self-registration must never be able to grant admin, since that's a straightforward privilege-escalation hole. Since that leaves no way to obtain an admin account at all, user-service creates one on startup from `ADMIN_USERNAME`/`ADMIN_EMAIL`/`ADMIN_PASSWORD` if none exists yet. This mirrors how Keycloak, Grafana, and Nextcloud handle the same bootstrapping problem: whoever deploys the container controls the first admin's credentials through the environment, rather than a value hardcoded into the image or a public endpoint that could mint one. If those variables aren't set, the app just skips the step (with a warning) instead of failing to start — it's a no-op in any environment that hasn't opted in.
